    ...
    @forelse($tasks as $task)
      <article class="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200">
        ...
        <div class="mt-5 flex flex-wrap items-center gap-3">
          <a href="{{ route('tasks.show', $task) }}" class="btn-link-strong inline-flex items-center gap-2 text-sm">
            Lihat tugas
            <i class="fa-solid fa-arrow-right text-xs"></i>
          </a>
          @if($canManageTasks)
            <a href="{{ route('tasks.edit', $task) }}" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-2xl bg-slate-100 px-4 py-2 text-xs font-black text-slate-700 transition hover:-translate-y-0.5 hover:bg-indigo-50 hover:text-indigo-700 hover:shadow-lg">
            ...
            <form method="POST" action="{{ route('tasks.destroy', $task) }}" onsubmit="return confirm('Hapus tugas ini? Jawaban siswa untuk tugas ini juga akan terhapus.');">
              @csrf
              @method('DELETE')
              <button class="inline-flex min-h-10 items-center justify-center gap-2 rounded-2xl bg-rose-600 px-4 py-2 text-xs font-black text-white shadow-lg shadow-rose-100 transition hover:-translate-y-0.5 hover:bg-rose-700 hover:shadow-xl" type="submit">
