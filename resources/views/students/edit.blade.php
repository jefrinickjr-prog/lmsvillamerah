      ...
      <div class="space-y-5">
        <div>
          ...
          <select name="delivery_mode" class="block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100" required>
            <option value="offline" @selected(old('delivery_mode', $student->delivery_mode ?? 'offline') === 'offline')>Tatap Muka / Offline</option>
            <option value="online" @selected(old('delivery_mode', $student->delivery_mode ?? 'offline') === 'online')>Live Streaming / Online</option>
          </select>
        </div>
        <div>
        ...
        <div>
          ...
          <select name="program_type" class="block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100" required>
            @foreach($programTypes as $value => $label)
              <option value="{{ $value }}" @selected(old('program_type', $student->program_type ?? 'gambar') === $value)>{{ $label }}</option>
            @endforeach
          </select>
        </div>
